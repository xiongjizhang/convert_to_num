...
public class ConvertToLinkNumHandler implements PojoRequestHandler<JSONObject, JSONObject> {
    @Override
    public JSONObject handleRequest(JSONObject eventObj, Context context) {
        ...
        String slotValue = slots.getString("联系号码.联系号码");
        String link_num = "";
        ...
        global_params.put("link_num", link_num);
        eventObj.put("global", global_params);
        if (link_num.length() > 0) {
